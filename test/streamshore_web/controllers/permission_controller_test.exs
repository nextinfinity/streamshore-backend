defmodule PermissionControllerTest do
  ...
  test "ban permission", %{conn: conn} do
    conn
    |> put(Routes.room_permission_path(conn, :show, "permissions", "ban-user"), %{
      permission: PermissionLevel.banned()
    })
  ...
  test "banned user can't join", %{conn: conn} do
    conn
    |> put(Routes.room_permission_path(conn, :show, "permissions", "ban-user2"), %{
      permission: PermissionLevel.banned()
    })
    ...
    connection =
      socket(StreamshoreWeb.UserSocket, "ban-user", %{user: "ban-user2", anon: true})
  ...
  test "mute permission", %{conn: conn} do
    conn
    |> put(Routes.room_permission_path(conn, :show, "permissions", "mute-user"), %{
      permission: PermissionLevel.muted()
    })
  ...
  test "muted user can't chat", %{conn: conn} do
    conn
    |> put(Routes.room_permission_path(conn, :show, "permissions", "mute-user2"), %{
      permission: PermissionLevel.muted()
    })
    ...
    {:ok, _, connection} =
      socket(StreamshoreWeb.UserSocket, "mute-user2", %{user: "mute-user2", anon: true})
      |> subscribe_and_join(StreamshoreWeb.RoomChannel, "room:permissions")
